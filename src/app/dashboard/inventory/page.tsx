  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-[var(--text-primary)]">Inventory & Catalog</h1>
          <p className="text-[var(--text-secondary)] text-sm mt-1">Manage billing items, professional services, and custom categories</p>
        </div>

        <div className="flex flex-wrap gap-3">
          <button
            onClick={() => setIsCategoryModalOpen(true)}
            className="flex items-center justify-center gap-2 py-2.5 px-4 bg-[var(--bg-surface)] border border-[var(--border-default)] hover:bg-[var(--bg-surface-alt)] text-[var(--text-secondary)] text-xs font-semibold rounded-xl transition-all cursor-pointer"
          >
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 py-2.5 px-5 bg-violet-600 hover:bg-violet-500 text-[var(--text-primary)] text-xs font-semibold rounded-xl transition-all hover:shadow-lg hover:shadow-violet-600/10 active:scale-[0.98] cursor-pointer"
          >
                  ...
                  <tr key={p.id} className="hover:bg-[var(--bg-surface-alt)] transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-lg bg-[var(--bg-input)] border border-[var(--border-default)] flex items-center justify-center text-violet-400 shrink-0">
                          <Package className="w-4.5 h-4.5" />
                        </div>
                        <div>
                          <p className="font-semibold text-[var(--text-primary)] text-sm">{p.name}</p>
                          <p className="text-xs text-[var(--text-muted)] max-w-sm mt-0.5 line-clamp-1">
                    ...
                    <td className="px-6 py-4">
                      {p.categories ? (
                        ...
                        <span className="text-[var(--text-muted)] text-xs italic">Uncategorized</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      {p.hsn_sac ? (
                        <span className="font-mono text-xs text-[var(--text-secondary)] bg-[var(--bg-surface-alt)] px-2 py-0.5 rounded">{p.hsn_sac}</span>
                      ) : (
      ...
      {isCategoryModalOpen && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleAddCategory} className="space-y-4">
              <div>
                <label className="block text-[var(--text-secondary)] text-xs font-medium mb-1.5">Category Name</label>
                <input
                  type="text"
                  required
              ...
              <button
                type="submit"
                className="w-full py-2.5 bg-violet-600 hover:bg-violet-500 text-[var(--text-primary)] font-semibold rounded-xl text-sm transition-all"
              >
                Create Category
              </button>
            </form>
            ...
            <form onSubmit={handleSaveProduct} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  <label className="block text-[var(--text-secondary)] text-xs font-medium mb-1.5">Item/Service Name</label>
                  <input
                    type="text"
                    required
                ...
                {/* HSN/SAC Code */}
                <div>
                  <label className="block text-[var(--text-secondary)] text-xs font-medium mb-1.5">HSN / SAC Code</label>
                  <input
                    type="text"
                    placeholder="e.g. 9988 or 4911"
                    value={prodHsn}
                    onChange={(e) => setProdHsn(e.target.value)}
                    className="w-full px-4 py-2.5 bg-[var(--bg-input)] border border-[var(--border-default)] rounded-xl text-[var(--text-primary)] focus:outline-none focus:ring-1 focus:ring-violet-600 focus:border-transparent text-sm font-mono"
                  />
                </div>
